<x-layouts.admin title="Pricing Rules & Rate Configuration | Admin">
    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Dynamic Pricing & Rate Configuration</h1>
            ...
            <div>
                <button onclick="document.getElementById('addRuleModal').classList.remove('hidden')" class="px-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs shadow-md transition-all flex items-center gap-2 cursor-pointer">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-3xl border border-slate-200/90 shadow-sm space-y-2">
                <span class="px-2.5 py-1 rounded-lg bg-amber-50 text-amber-700 font-extrabold text-[10px] uppercase tracking-wider">Peak Windows</span>
                <h3 class="text-2xl font-extrabold text-slate-900">{{ $pricingRules->where('rule_type', 'peak')->count() }} Active</h3>
                <p class="text-xs text-slate-500">Configured peak rate surcharges (e.g. 17:00–22:00 weekdays & weekends).</p>
            </div>

            <div class="bg-white p-6 rounded-3xl border border-slate-200/90 shadow-sm space-y-2">
                <span class="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 font-extrabold text-[10px] uppercase tracking-wider">Seasonal Adjustments</span>
                <h3 class="text-2xl font-extrabold text-slate-900">{{ $pricingRules->where('rule_type', 'seasonal')->count() }} Active</h3>
                <p class="text-xs text-slate-500">Date-range specific seasonal adjustments (e.g. Summer & Holiday peak rates).</p>
            </div>

            <div class="bg-white p-6 rounded-3xl border border-slate-200/90 shadow-sm space-y-2">
                <span class="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-extrabold text-[10px] uppercase tracking-wider">Discount Rules</span>
                <h3 class="text-2xl font-extrabold text-slate-900">{{ $pricingRules->where('rule_type', 'discount')->count() }} Active</h3>
        ...
        <div class="bg-white rounded-3xl border border-slate-200/90 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <h2 class="text-base font-extrabold text-slate-900">Tenant Pricing Rules</h2>
                <span class="text-xs text-slate-500 font-medium">Applied in order of calculation engine priority</span>
            </div>
                            ...
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase
                                        {{ $rule->rule_type === 'peak' ? 'bg-amber-100 text-amber-800' : '' }}
                                        {{ $rule->rule_type === 'seasonal' ? 'bg-indigo-100 text-indigo-800' : '' }}
                                        {{ $rule->rule_type === 'discount' ? 'bg-emerald-100 text-emerald-800' : '' }}
                                        {{ $rule->rule_type === 'off_peak' ? 'bg-slate-100 text-slate-800' : '' }}">
                                ...
                                <td class="px-6 py-4 text-slate-600">
                                    @if ($rule->start_time && $rule->end_time)
                                        <code>{{ $rule->start_time }} – {{ $rule->end_time }}</code>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1.5 text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full font-extrabold text-[10px] uppercase">
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    <form action="{{ route('admin.pricing.toggle', $rule->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-slate-600 hover:text-brand-600 font-bold hover:underline">
    ...
    <div id="addRuleModal" class="hidden fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs items-center justify-center p-4">
        <div class="bg-white rounded-3xl max-w-lg w-full p-8 space-y-6 shadow-2xl border border-slate-200">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                ...
                <button onclick="document.getElementById('addRuleModal').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 font-bold">✕</button>
            </div>
            ...
            <form action="{{ route('admin.pricing.store') }}" method="POST" class="space-y-4 text-xs">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="e.g. Evening Peak Surcharge" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 font-medium">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block font-bold text-slate-700 uppercase tracking-wider mb-1">Rule Type</label>
                        <select name="rule_type" required class="w-full px-4 py-3 rounded-xl border border-slate-300 font-medium">
                    ...
                    <div>
                        ...
                        <select name="discount_type" class="w-full px-4 py-3 rounded-xl border border-slate-300 font-medium">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="adjustment_type" required class="w-full px-4 py-3 rounded-xl border border-slate-300 font-medium">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="adjustment_value" required placeholder="e.g. 20 or -15" class="w-full px-4 py-3 rounded-xl border border-slate-300 font-medium">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="start_time" placeholder="17:00" class="w-full px-4 py-3 rounded-xl border border-slate-300 font-medium">
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 uppercase tracking-wider mb-1">End Time (HH:MM)</label>
                        <input type="text" name="end_time" placeholder="22:00" class="w-full px-4 py-3 rounded-xl border border-slate-300 font-medium">
                    </div>
                </div>

                <div class="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
                    <button type="button" onclick="document.getElementById('addRuleModal').classList.add('hidden')" class="px-5 py-3 rounded-xl border border-slate-200 font-bold text-slate-600 hover:bg-slate-50">Cancel</button>
                    <button type="submit" class="px-6 py-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold shadow-md">Save Pricing Rule</button>
                </div>
            </form>
        </div>
    </div>
